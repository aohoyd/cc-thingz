# Hunk Review (vendored)

> Vendored copy of the hunk CLI's bundled skill, taken from hunk v0.17.3 (`hunk skill path`).
> To refresh after a hunk upgrade: replace everything below this header with the contents of `$(hunk skill path)` (minus its frontmatter) and update the version above.
...
- `comment apply` payload items require `filePath`, `summary`, and exactly one target such as `hunk`, `hunkNumber`, `oldLine`, or `newLine`; items also accept optional `rationale` and `author`
...
- `comment clear` removes live agent comments only by default; `--include-user` / `--all` also remove human notes created in the TUI
