...
- **Step 1** — asks focused questions batched into one AskUserQuestion call (goal, scope, constraints), skipping anything a design doc or the session already settled; the plan title is derived, not asked, and the testing approach (TDD vs regular) is chosen by Claude per task type — a user-stated preference always overrides
